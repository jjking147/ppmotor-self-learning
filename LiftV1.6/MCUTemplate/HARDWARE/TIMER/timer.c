...
void Modbus_Master_Timer_Init(int ms) // 设定过几ms触发一次溢出中断
{
	TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;

	MODBUS_MASTER_TIM_CLK_EN; /// 使能时钟 TIM7

	TIM_TimeBaseInitStructure.TIM_Period = (ms << 1) - 1; // 设置定时器的计时时间
	;													  // 自动重装载值
	TIM_TimeBaseInitStructure.TIM_Prescaler = (84000 / 2 - 1);
	;																// 定时器分频
...
{
	if (TIM_GetITStatus(MODBUS_MASTER_TIM, TIM_IT_Update) == SET) // 溢出中断，将Master_State设成MODBUS_TIMEOUT，并且停止TIM7的计数
	{
		Master_Set_Timeout();
...
void Delay_Timer_Init(void)
{
	...
	TIM_ITConfig(DELAY_TIM, TIM_IT_Update, ENABLE); // 允许定时器更新中断，但是并不会进入中断函数，因为NVIC里没有开启对应的中断入口
