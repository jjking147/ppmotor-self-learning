...
{
  /* Check the parameters */
  assert_param(IS_USART_ALL_PERIPH(USARTx)); // 检验是否用的是USART
  assert_param(IS_USART_DATA(Data));         // 检验传送的数据是否小于9位，但是检验作废了，不检验
  ...
  uint16_t USART_ReceiveData(USART_TypeDef * USARTx)
  {
    ...
    /* Receive Data */
    return (uint16_t)(USARTx->DR & (uint16_t)0x01FF); // 这里相当于在读取USARTx->DR的值了
  }
